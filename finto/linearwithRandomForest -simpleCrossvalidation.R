...
for (i in 1:3){
  ...
  X_build <- subset(train,(Applicant_Gender==i),select = -c(ID) )
   model_rf <- randomForest(formula = LnvalidatedRf, data=X_build,ntree=576,mtry=2,nodesize=1)
  
  X_Train<- subset(test,(test$Applicant_Gender==i) )
  pred_rf <- predict(model_rf, X_Train)
  
  if(i==1){
    error[i]=c("Male"=max(model_rf$err.rate))
  }else if(i==2){
    error[i]=c("Female" =max(model_rf$err.rate))
  }else{
    error[i]=c("others"=max(model_rf$err.rate))
  }
  
  
}
errdf<-data.frame(error)
